            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <div>
                    ...
                    <select 
                        ...
                        onChange={e => setSelectedMailingId(e.target.value)} 
                        className="w-full border p-2.5 rounded-lg bg-white outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    >
                ...
                <div>
                    ...
                    {isScheduling ? (
                        <input 
                            ...
                            className="w-full border p-2 rounded-lg bg-white outline-none focus:ring-2 focus:ring-blue-500 text-sm animate-in fade-in" 
                        />
                    ) : (
                        <div className="w-full border p-2 rounded-lg bg-slate-50 text-slate-400 text-sm text-center border-dashed">
                            Disparo Manual
                        </div>
                    )}
                </div>
                <button 
                    type="submit" 
                    className="text-white px-4 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 hover:opacity-90 bg-blue-600"
                >
...
export const QueueCard = ({ queue, statusBadge, onToggleStatus, onDeleteQueue, onDeleteBlock, onEditBlock, onAddBlock, onBlockClick }) => {
            ...
            <div className={`p-4 border-b flex justify-between items-center ${hasErrorActive ? 'bg-red-100/50' : 'bg-slate-50/50'}`}>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {(!queue.blocked && queue.status !== 'completed') && (
                        <button onClick={onToggleStatus} className={`p-2 rounded-full hover:bg-slate-200 ${queue.hasError ? 'text-red-700' : 'text-slate-600'}`}>
                            {queue.status === 'running' ? <Pause size={22} fill="currentColor" /> : <Play size={22} fill="currentColor" className="text-green-600" />}
                        </button>
                    )}
                    <div className="w-px h-6 bg-slate-300 mx-1"></div>
                    <button onClick={onDeleteQueue} className="p-2 rounded-full hover:bg-red-100 text-slate-400 hover:text-red-600">
                        <Trash2 size={18} />
                    </button>
                </div>
            </div>
            <div className="p-4 overflow-x-auto flex items-center gap-4">
                {queue.blocks.map((block, index) => (
                    <React.Fragment key={block.id}>
                        <BlockCard 
                            block={block} 
                            isActive={isRunning && index === (queue.activeBlockIndex || 0)} 
                            canEdit={!isRunning}
                ...
                {/* REGRA: Só aparece o botão de Add Bloco se a fila não tiver sido finalizada e não estiver bloqueada */}
                {queue.status !== 'completed' && !queue.blocked && (
                    <button 
                        onClick={onAddBlock} 
                        className="flex-shrink-0 h-44 w-40 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center text-slate-400 hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all gap-2 group"
                    >
...
export const BlockCard = ({ block, onDelete, onEdit, onClick, isActive, canEdit }) => {
    ...
    else if (isActive) blockClass = 'bg-white border-blue-500 ring-2 ring-blue-200 shadow-lg scale-105 z-10';
        ...
        <div onClick={onClick} className={`flex-shrink-0 w-72 border rounded-lg p-4 relative group cursor-pointer transition-all hover:shadow-lg ${blockClass}`}>
            ...
            {canEdit && (
                <div className="absolute -top-3 -right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                    <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="bg-blue-500 text-white rounded-full p-1.5 shadow-md hover:bg-blue-600">
                        <Edit2 size={14} />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="bg-red-500 text-white rounded-full p-1.5 shadow-md hover:bg-red-600">
                        <X size={14} />
                    </button>
                </div>
            )}
            ...
            <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-200">
                <span className={`text-sm font-bold truncate ${block.hasError ? 'text-red-800' : 'text-slate-700'}`}>
                    {block.waba.description}
                </span>
                <div className="flex items-center gap-2">
                    {isActive && <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded animate-pulse border border-blue-200">ATIVO</span>}
                    <div className="text-[10px] font-bold bg-white text-slate-600 px-2 py-0.5 rounded border border-slate-200">
                        {block.config.maxPercentage}% CAP
                    </div>
                </div>
            </div>
            <div className="flex justify-between items-center mb-2 bg-white/60 p-2 rounded border border-slate-200">
                <span className="text-xs font-mono text-slate-600">{block.line.phone}</span>
            </div>
            <div className="bg-white/60 p-2 rounded border border-slate-200 mb-2">
                <span className="text-xs font-bold text-slate-700 truncate block">{block.template.name}</span>
            </div>
            <div className="flex justify-between mt-3 text-xs font-bold">
                ...
                {block.status === 'completed' && !block.hasError && <span className="flex items-center gap-1 text-green-600"><CheckCircle size={14} /> Finalizado</span>}
            </div>
        </div>
    );
};